...
public class Test {
	...
	public static void getuser_1() {
		...
		JavaRDD<String> result=group.map(new Function<Tuple2<String,Iterable<Tuple2<String,String>>>, String>() {
        ...
        List<String> reslist = result.distinct().collect();  
	...
	public static void getuser() {
		...
		JavaPairRDD<String, String> ud =address .union(user).distinct();
		//JavaPairRDD<String, String> ud = address.intersection(user);
		JavaPairRDD<String, Iterable<String>> group = ud.groupByKey();
		//JavaPairRDD<String, Iterable<String>> dis = group.distinct();
		JavaPairRDD<String, String> out = group
				.mapToPair(new PairFunction<Tuple2<String, Iterable<String>>, String, String>() {
					...
					public Tuple2<String, String> call(
							...
							throws Exception {
						Iterator<String> value = arg0._2().iterator();
						String str = null;
						String add = "";
						String user = "";
						while (value.hasNext()) {
		...
		// ud.saveAsTextFile("hdfs://hadoop:9000/usr/spark/ud");
		for (Tuple2<String, String> ss : out.collect()) {
			System.out.println(ss._1() + "    " + ss._2());
		}
	...
	public static void getUserAddressTimes(Long IntervalTime) {
		...
		// 先对数据分组
				...
				JavaPairRDD<String, Iterable<String>> result_user_Adds=invertedIndex(result_Add_users);
				
				
				
		     		for (Tuple2<String, String> tuple2 : result_Add_users.distinct().collect()) {
		     			System.out.println(tuple2._1() + "    " + tuple2._2());
		     		}
	...
	public static JavaPairRDD<String, Iterable<String>> invertedIndex(JavaPairRDD<String, String> result_Add_users) {
	    ...
	    JavaPairRDD<String, String> pairs = result_Add_users.flatMapToPair(new PairFlatMapFunction<Tuple2<String,String>, String, String>() {
			...
			public Iterable<Tuple2<String, String>> call(
				...
				List results = new ArrayList();
			      ...
			      List<String> users = Arrays.asList(file._2().split(","));
			    	      String doc = file._1();
			      ...
			      for (String hashtag : hashtags) {
			        Tuple2<String, String> result = new Tuple2<>(hashtag, doc);
			        results.add(result);
			      }
				return results;
			}
	    		...
	    		/**
	    		 * .flatMapToPair(new PairFlatMapFunction<String, String, Tweet>() {
