...
import org.apache.cassandra.thrift.Cassandra.add_args;
...
public class Test12 {
	...
	public static JavaPairRDD<String, Long> CalIntervalTime(JavaPairRDD<String, Iterable<Long>> sort,Long IntervalTime){
		// 对分组后的数据进行KV对换
				JavaPairRDD<String, Long> timeall = sort.mapToPair(t->{
					Iterator<Long> list = t._2().iterator();
					String sp = "";
					while (list.hasNext()) {
						sp += list.next() + "|";
					}
					...
					return new Tuple2<String, Long>(t._1(), times);
				});
				return timeall;
	...
	public static JavaPairRDD<String, Long> getTime(Long IntervalTime) {
		...
		JavaRDD<String> text = sc.textFile(UserPath);
		// 先对数据分组
		JavaPairRDD<String, Long> line = text.flatMapToPair(s->{
			String[] str = s.split("\\|");
			long time = formDateStringToLong(str[3]);
			String idlocal = str[0] + "        " + str[1] + "|"+ str[2];
		...
		return CalIntervalTime(sort,IntervalTime);
	...
	public static void getUserAddressTimes(Long IntervalTime) {
		...
		// 先对数据分组
				...
				// 对分组后的数据进行KV对换
					JavaPairRDD<String, Long> timeall =CalIntervalTime(result_user_Add_times, IntervalTime);
